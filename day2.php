...
function part2(array $data, $pattern, $count = 0)
{
    foreach ($data as $str) {
        preg_match($pattern, $str, $matches);
        $validatingPattern = sprintf("/(?:.){%s}(.)(?:.){%s}(.).*/", $matches['min'] - 1, $matches['max'] - $matches['min'] - 1);
        preg_match($validatingPattern, $matches['password'], $letter);
        if ($letter[1] != $letter[2] && ($letter[1] == $matches['letter'] || $letter[2] == $matches['letter'])) {
            $count++;
        }
    }
    return $count;
